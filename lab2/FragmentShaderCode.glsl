...
uniform vec3 DirLight_shininess;
uniform float DirLight_shininess2;
...
vec3 CalcDirLight(DirLight light, vec3 normal, vec3 viewDir)
{
    ...
    return DirLight_shininess2*DirLight_shininess*(ambient + diffuse + specular);
}
...
vec3 CalcSpotLight(SpotLight light, vec3 normal, vec3 fragPos, vec3 viewDir)
{
    ...
    if(spotlight==1){
        ...
        return 0*(ambient + diffuse + specular);
    }
